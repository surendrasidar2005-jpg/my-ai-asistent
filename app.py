...
GROQ_MODELS = [
    "llama3-groq-70b-8192-tool-use-preview",  # Tool-use optimized (fixes 400 error)
    "llama3-groq-8b-8192-tool-use-preview",   # Fallback tool model
    "llama-3.3-70b-versatile",                 # General fallback
...
async def execute_tool(name: str, args: dict) -> str:
    try:
        if name == "web_search":
            ...
            # Safety check for factorial to prevent DoS
            if "factorial" in expr:
                import re
                nums = re.findall(r'factorial\((\d+)\)', expr)
                for n in nums:
                    if int(n) > 1000:
                        return "❌ Error: Factorial too large (max 1000)"
...
async def agent_stream(messages: list):
    ...
    async with httpx.AsyncClient(timeout=120) as client:
        while iteration < MAX_ITERATIONS:
            ...
            # FIX: Handle 400 tool_use_failed — retry without tools
            if resp.status_code == 400 and "tool_use_failed" in resp.text:
                payload_no_tools = {k: v for k, v in payload.items()
                ...
                try:
                    ...
                    yield f"data: {json.dumps({'type':'error','text':f'Retry failed: {retry_err}'})}\n\n"
                    return
...
async def agent(req: AgentRequest, request: Request):
    ...
    for h in req.history[-15:]:
        # Allow user, assistant, and tool roles to maintain sequence integrity
        if h.get("role") in ("user", "assistant", "tool"):
            msg_obj = {"role": h["role"], "content": str(h.get("content") or "")}
            if h.get("tool_call_id"):
                msg_obj["tool_call_id"] = h["tool_call_id"]
            if h.get("tool_calls"):
                msg_obj["tool_calls"] = h["tool_calls"]
...
async def delete_file_api(filename: str):
    ...
    if path.exists():
        path.unlink()
        return {"ok": True, "message": f"File {filename} deleted"}
    raise HTTPException(404, "File not found")
